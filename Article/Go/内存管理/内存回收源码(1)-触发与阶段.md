...
Go 运行时垃圾回收主要落在 `runtime/mgc.go`、`mgcmark.go`、`mgcsweep.go`、`mbarrier.go` 等。本系列拆成三篇，便于按调用链阅读：
...
一条主线：**触发 → STW 准备 → 并发标记（mutator + worker + assist + 写屏障）→ 标记收尾 STW → 并发 sweep**。三篇顺序读，可沿分配（`mallocgc`）与标记（`gcDrain`）对照源码。
