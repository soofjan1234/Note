...
def remove_front_matter(markdown_content):
    """
    仅移除“文件开头”的 YAML front matter。

    注意：正文里也可能会使用 `---` 作为分隔线（水平线），不应该误删。
    """
    # 只匹配字符串开头的 front matter：第一行必须是单独的 `---`
    # 结束也要求是单独的 `---` 行。
    front_matter_pattern = r"\A---\s*[\r\n]+[\s\S]*?[\r\n]+---\s*(?:[\r\n]+|$)"
    return re.sub(front_matter_pattern, "", markdown_content, flags=re.DOTALL)
